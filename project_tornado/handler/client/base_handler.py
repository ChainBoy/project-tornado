...
class BaseHandler(tornado.web.RequestHandler):
    # config = {'host':"10.10.10.123", 'db':'voice', 'user':'dev', 'passwd':'kaimen', 'charset':"utf8"}
    # config = {'host': "127.0.0.1", 'db': 'voice', 'user': 'root', 'passwd': 'kaimen', 'charset': "utf8"}
    # _return_json = ApiEntity()
    ...
    def render(self, template_name, **kwargs):
        ...
        if not kwargs.get('user'):
            try:
                kwargs['user'] = self.get_user_cookie().get('user')
    ...
    def set_error(self, error_code=0):
        code = str(error_code)
        message = ENUM_ERROR_CODE.get(
            code, "UnKnown Error, Please message to Admin.")
    ...
    def get_argument(self, name, default="", strip=True, name_type=str):
        ...
        :return: 返回转化后的值.
        """
        result = super(BaseHandler, self).get_argument(
        ...
        try:
            ...
            logging.error(
                'base_handler get argu:E:(%s). e_str:%s' % (e, e_str))
            result = default
        return result

    def _get_request_json(self, field_key="data"):
        "获取json格式的数据.已经转为dict"
        body_json = self.request.headers.get(
            "data", "") or self.get_argument(field_key)
        try:
            body_dict = simplejson.loads(body_json)
            return body_dict

        except simplejson.JSONDecodeError:
            logging.error("body is not json data? &%s=%s" %
                          (field_key, body_json))
            return {}
    ...
    def is_admin(self):
        ...
        cookie = {"u_type": 1}
        if cookie:
            u_type = cookie.get('u_type', '')
            return True if u_type == 1 else False
        return False
    ...
    def dump(self):
        ...
        :return:
        """
        uri = self.get_argument('refer', '')
        if not uri or self.request.uri == uri:
            return
        self.redirect('/?refer=%s' % uri)
